...
cd C:\Users\user\claude-code-setup
git branch -M main
git remote add origin https://github.com/你的帳號/claude-code-setup.git
...
```
https://raw.githubusercontent.com/你的帳號/claude-code-setup/main/install-claude-code.ps1
```

所以發給學生的**一行指令**是：

```powershell
irm https://raw.githubusercontent.com/你的帳號/claude-code-setup/main/install-claude-code.ps1 | iex
```

> 把 `你的帳號` 換成你的 GitHub 帳號。這行貼進投影片 / 課程頁即可。

### 記得順手改兩個地方的佔位字（換成你真實帳號）
1. `README.md` 裡的 `<你的GitHub帳號>/<你的repo>`
2. `handout.html` 裡的 `你的帳號`（第一個 code 區塊，以及最下面 `<script>` 的 `text` 變數）
