...
- Chapters ③–⑭ follow the same 3-file pattern: `first-magic` ③, `how-ai-works` ④, `what-is-agent` ⑤, `claudemd` ⑥, `command-map` ⑦, `skill` ⑧, `mcp` ⑨, `subagent` ⑩, `project` ⑪, `deploy` ⑫, `showcase` ⑬, `resources` ⑭.
- `images/` — screenshots used by chapter ⑬ (showcase). This is the ONE exception to the "no external assets" rule below: real-project screenshots are too large to inline as data URIs across three language files, so the three `showcase*.html` pages reference `images/showcase-*.jpg` relatively. All screenshots are privacy-sanitized (kids' names replaced) before being committed — keep it that way when regenerating them.
